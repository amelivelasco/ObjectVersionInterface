...
function interCellSegmentsConflict(
  candidate,
  existing
) {
  if (
  ...
  /*
   * Parallel wires may almost touch.
   */
  const parallelClearance =
    strokeWidth + 0.25;
   ...
   *
   * Block only a real geometric intersection.
   * Do not use an 8px expanded rectangle here,
   * because that prevents routes from going around
   * the endpoint of another wire.
   */
  const horizontal =
    candidateHorizontal
      ? candidate
