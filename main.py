...
def main():
    ...
    original_netlist_path, layout_path = select_project(base_dir)
    ...
    top_cell_name = read_custom_compiler_cell_name(original_netlist_path) or str(circuit.TOP.name)

    schematic.attach_original_values(original_circuit, circuit, paths["sol"].exists())

    klayout_exp, inductex_exp = schematic.setup_exporters(
        circuit, layout_path, paths["output_dir"], combined_layout_map, paths["sol"]
    )

    first_level_layout_cells = schematic.prepare_layout_mapping(circuit, klayout_exp)

    schematic.generate_inductex_files(
        parser, klayout_exp, inductex_exp,
        {"layout_path": layout_path, "paths": paths, "top_cell_name": top_cell_name}
    )

    klayout_exp.cover_cell_with_layer()

    schematic.generate_schematic_files(
        parser, circuit, netlist_path, paths, first_level_layout_cells, top_cell_name
    )
